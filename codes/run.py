...
def parse_args(args=None):
    ...
    parser.add_argument('--data_path', type=str, default=None) #"../data/wn18rr"
    ...
    parser.add_argument('-save', '--save_path', default="../models/TransE", type=str)
    parser.add_argument('--max_steps', default=0, type=int)
    parser.add_argument('--warm_up_steps', default=None, type=int)
    parser.add_argument('--freeze_steps', default=60000, type=int)
...
def save_model(model, optimizer, save_variable_list, args):
    ...
    if args.model.__contains__('ellipse'):
        ellipse = model.ellipse.detach().cpu().numpy()
        np.save(
            os.path.join(args.save_path, 'ellipse'),
            ellipse
        )
    ...
    with open(file_path) as fin:
        for line in fin:
            ...
            try:
                ...
                exit()
            try:
                triples.append((entity2id[h.zfill(8)], relation2id[r.zfill(8)], entity2id[t.zfill(8)]))
            except:
...
def set_logger(args):
    ...
    elif args.do_sub_test:
        print('-'*20, 'do_sub_test ', args.do_sub_test, '-'*20)
...
def log_metrics_rel(mode, step, metrics_rel, id2relation):
    '''
    Print the evaluation logs for every relation
    '''
    print('#'*50, '\n', '#'*50, '\nPrint the evaluation logs for every relation:')
    for rel in metrics_rel:
        logging.info('%s: ' % id2relation[rel])
...
def main(args):
    ...
    if args.countries:
        regions = list()
        with open(os.path.join(args.data_path, 'regions.list'), encoding='utf-8') as fin:
            for line in fin:
                region = line.strip()
    ...
    if args.do_sub_test:
        ...
        for i in filelist:
            if not 'test' in i:
                continue
            sub_test[i] = read_triple(os.path.join(filepath, i), entity2id, relation2id)
            logging.info(i+'#test: %d' % len(sub_test[i]))
        #exit()
    ...
    if args.do_test:
        logging.info('Evaluating on Test Dataset...')
        metrics, metrics_rel = kge_model.test_step(kge_model, test_triples, all_true_triples, args)
        log_metrics('Test', step, metrics)
        log_metrics_rel('Test', step, metrics_rel, id2relation)
